...
public static class PathApproximator
{
    ...
    public static List<Vector2> ApproximateLagrangePolynomial(List<Vector2> controlPoints)
    {
        ...
        double[] weights = BarycentricWeights(controlPoints);
        ...
        for (int i = 0; i < num_steps; i++)
        {
            ...
            float y = (float)BarycentricLagrange(controlPoints, weights, x);
    ...
    private static bool bezierIsFlatEnough(Vector2[] controlPoints)
    {
        for (int i = 1; i < controlPoints.Length - 1; i++)
            if ((controlPoints[i - 1] - 2 * controlPoints[i] + controlPoints[i + 1]).sqrMagnitude > bezier_tolerance * bezier_tolerance * 4)
                return false;
    ...
    private static double[] BarycentricWeights(List<Vector2> points)
    {
        int n = points.Count;
    ...
    private static double BarycentricLagrange(List<Vector2> points, double[] weights, double time)
    {
